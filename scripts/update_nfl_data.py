...
if not game_stats_summary_df.empty:
    game_stats_summary_df.to_sql('nfl_game_stats_summary', sql_connection, if_exists='append', index=False)

# Table With Ratings
current_ratings_ids = set(
    pd.read_sql_query('select game_id from nfl_game_ratings', sql_connection)['game_id'].astype(str).tolist()
)
games_from_summary = set(
    pd.read_sql_query('select game_id from nfl_game_stats_summary', sql_connection)['game_id'].astype(str).tolist()
)
new_ids_from_games_table = games_from_summary - current_ratings_ids
...
rating_df['game_id'] = rating_df['game_id'].astype(str)
rating_df = rating_df[rating_df['game_id'].isin(new_ids_from_games_table)].copy()
